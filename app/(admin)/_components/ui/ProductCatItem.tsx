...
interface ProductCatItemProps {
  data: any;
  onDelete: () => void;
}

const ProductCatItem = ({ data, onDelete }: ProductCatItemProps) => {
  ...
  const handleDelete = async () => {
    ...
    await axios
      .delete(
        `${process.env.NEXT_PUBLIC_BACKEND_API_URL}product/categories/${data.id}/delete`,
        {
          headers: {
  ...
  return (
    <div className="w-full p-4 bg-white rounded-md flex items-center justify-between mb-3">
      ...
      <div className="flex gap-3 items-center">
        <Link
          href={`/dashboard/products/categories/edit/${data.id}`}
